package A8RAGCACGHTEPlant20260715
  extends Modelica.Icons.Package;
  // 单机场景共用官方物理层，控制器只通过标准参考、状态和分配边界接口接入。
  ...
  equation
    ...
    // 场景诊断保留物理参数比例和分配上界，供Syslab统一复算。
    scenarioDiagnostics={
      quadChassisTest17_1.lift_cofficient/0.002,
      quadChassisTest17_1.body.m/0.159504,
      quadChassisTest17_1.body.I_11/0.00010556,
      quadChassisTest17_1.body.m,
      quadChassisTest17_1.body.I_11,
      quadChassisTest17_1.body.I_22,
      quadChassisTest17_1.body.I_33,
      0,0,qLimit,0,0,0,0,0,0};
  end PlantBase;
  // Scene04是正式悬停复现场景，也是发布前快速验证入口。
  model Scene04
    extends PlantBase(quadChassisTest17_1(lift_cofficient=0.002,body(m=0.159504,I_11=0.00010556,I_22=0.00010556,I_33=0.00010556),propellers1(m=0.000913171,I_11=1.59662e-07,I_22=1.59594e-07,I_33=3.16359e-07),propellers2(m=0.000913171,I_11=1.59662e-07,I_22=1.59594e-07,I_33=3.16359e-07),propellers3(m=0.000913171,I_11=1.59662e-07,I_22=1.59594e-07,I_33=3.16359e-07),propellers4(m=0.000913171,I_11=1.59662e-07,I_22=1.59594e-07,I_33=3.16359e-07)));
  ...
  model Scene02
    extends PlantBase;
    QuadrotorModel.PathPlanning.CirclePath climbePath(
      ramp(height=20),sine(f=0.05),cosine(f=0.05,startTime=10,phase=0));
    A8FormalRAGCACGHTE_20260715 controller 
      annotation(Placement(transformation(origin={-15,0},extent={{-15,-15},{15,15}})),__MWORKS(SECInstance=true));
    Modelica.Blocks.Interfaces.RealOutput controllerDiagnostics[16] 
      annotation(Placement(transformation(origin={5,-35},extent={{-5,-5},{5,5}})));
  ...
  equation
    ...
    connect(controller.diagnostics,controllerDiagnostics) 
      annotation(Line(points={{0,-8},{5,-8},{5,-35}},color={0,0,127}));
    referenceVector={climbePath.position_command[1],climbePath.position_command[2],climbePath.position_command[3],
      if time<10 then 0 else -3*0.1*Modelica.Constants.pi*sin(0.1*Modelica.Constants.pi*(time-10)),
      if time<10 then 0 else 3*0.1*Modelica.Constants.pi*cos(0.1*Modelica.Constants.pi*(time-10)),
      if time<150 then 20/150 else 0,
      if time<10 then 0 else -3*(0.1*Modelica.Constants.pi)^2*cos(0.1*Modelica.Constants.pi*(time-10)),
      if time<10 then 0 else -3*(0.1*Modelica.Constants.pi)^2*sin(0.1*Modelica.Constants.pi*(time-10)),
      0,0,0};
    annotation(experiment(Algorithm=Dassl,StartTime=0,StopTime=50,Tolerance=0.0001,NumberOfIntervals=5000));
  end Scene02;
  model Scene03
